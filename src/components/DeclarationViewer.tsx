...
import { Save, Download, FileJson, FileSpreadsheet, FileCode } from 'lucide-react';
...
export default function DeclarationViewer({ data }: DeclarationViewerProps) {
    ...
    const handleDownload = (format: 'json' | 'xlsx' | 'xml') => {
        if (format === 'json') {
            ...
            alert('XML formatı henüz aktif değil.');
        }
    };
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mt-8">
            <div className="flex items-center justify-between mb-6 border-b pb-4">
                <h2 className="text-2xl font-bold text-gray-800">Beyanname Taslağı</h2>
                <div className="flex gap-2">
                    <button onClick={() => handleDownload('json')} className="p-2 text-gray-600 hover:bg-gray-100 rounded" title="JSON İndir"><FileJson size={20} /></button>
                    <button onClick={() => handleDownload('xlsx')} className="p-2 text-green-600 hover:bg-green-50 rounded" title="Excel İndir"><FileSpreadsheet size={20} /></button>
                    <button onClick={() => handleDownload('xml')} className="p-2 text-orange-600 hover:bg-orange-50 rounded" title="XML İndir"><FileCode size={20} /></button>
                </div>
            </div>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {editableData.esya_listesi?.map((item: any, i: number) => (
                                <tr key={i} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-3 font-mono text-blue-600">{item.gtip}</td>
